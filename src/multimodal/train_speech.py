"""Training script for speech models.

Training, evaluation, and tuning scheme:

1. Use `python train_speech.py [options]` to build, train, and validate models.
...
3. To change base model parameters, either edit the param dicts in speech.py, or 
   save the base params as json (hint: `--save-base-params`), edit the file, and 
   supply the model directory containing the new params file to override the 
   default base params.
...
TRAIN_OPTION_ARGS = [
    ...
    {   
        "varname": 'decay_rate',
        "names": ['-dr', '--decay-rate'],
        "help": "Exponential decay of learning rate per epoch (default: 1.0 - "
                "no decay)",
        "choices": None,
        "default": 1.0
    },
...
def check_arguments():
    """Check command line arguments for `python train_speech.py`."""
    ...
    parser.add_argument('--model-version',
                        type=str,
                        help="Type of speech model to build and train",
                        choices=speech.MODEL_VERSIONS,
                        required=True)
    parser.add_argument('--model-dir', 
                        type=os.path.abspath,
                        help="Path to store and load speech models "
    ...
    parser.add_argument('--save-base-params',
                        ...
                        help="Store the base parameters of the selected "
                             "speech model (as {}/{}) and exit".format(
    ...
    parser.add_argument('--train-set',
                        type=str,
                        help="Dataset to use for pre-training speech model "
                             "(defaults to '{}')".format('flickr-audio'),
                        choices=['flickr-audio', 'tidigits'],
                        default='flickr-audio')
    parser.add_argument('--feats-type',
                        type=str,
                        help="Type of extracted features (defaults to '{}')"
                             "".format('mfcc'),
                        choices=['mfcc', 'fbank'],
                        default='mfcc')
    parser.add_argument('--val-size',
                        ...
                        help="Number of validation examples to select from "
                             "the train dataset (defaults to {} and is only "
                             "applicable to tidigits)".format(5000),
                        default=5000)  # ~20% of tidigits
    parser.add_argument('--n-padded',
                        type=int,
                        help="Length to which speech segments will be "
                             "padded/truncated (default is {}).".format(80),
                        default=80)
    parser.add_argument('--center-padded',
                        action='store_true',
                        help="Pad/truncate speech segments with center padding "
                             "(defaults to padding on end of segments)")
    ...
    parser.add_argument('--balanced-batching',
                        ...
                        help="Sample balanced batches of P concept classes and "
                             ...
                             "normal batching; Overrides --batch-size with "
                             "P_batch*K_batch)")
    ...
    # --------------------------------------------------------------------------
    # Speech model hyperparameters:
    # --------------------------------------------------------------------------
    for model_param in speech.base_model_dict.keys():
...
def main():
    ...
    model_params['conv_input_shape'] = [n_filters, n_padding, 1]  # conv reshape
    ...
    with open(model_params_path, 'w') as fp:
        logging.info("Writing model parameters to file: {}".format(
            model_params_path))
    ...
    if ARGS.train_set == 'flickr-audio':  # load Flickr-Audio (default) train set
        logging.info("Training speech model on dataset: {}".format('flickr-audio'))
    ...
    with tf.device('/cpu:0'):
        ...
        x_train = np.swapaxes(x_train, 2, 1)  # switch to shape (n_filters, n_pad)
        ...
        x_val_split, y_val_split, z_val_split = (
            data.make_train_test_split(x_val_with_depth,
                                       y_val_placeholder,
                                       z_val_placeholder,
                                       test_ratio=0.5,
                                       shuffle=True,
    ...
    with tf.Session() as sess:
        ...
        for image, label, speaker in zip(*support_batch):
            utils.save_image(np.squeeze(image, axis=-1), filename=os.path.join(
                model_dir, 'train_images', '{}_{}_{}_{}_{}.pdf'.format('support', 'l',
                label.decode("utf-8"), 's', speaker.decode("utf-8"))), cmap='inferno')
        for image, label, speaker in zip(*query_batch):
            utils.save_image(np.squeeze(image, axis=-1), filename=os.path.join(
                model_dir, 'train_images', '{}_{}_{}_{}_{}.pdf'.format('query', 'l', 
                label.decode("utf-8"), 's', speaker.decode("utf-8"))), cmap='inferno')
