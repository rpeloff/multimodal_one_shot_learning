"""Build functions for unimodal speech models.
...
# BASE MODEL params - these are updated by model specific parameters
base_model_dict = {
    # ----------------------
    ...
    # Other model options:
    # --------------------
    "x_input_shape": [None, None, None, 1],  # define model input shape
    "conv_input_shape": [None, None, 1],  # define conv input shape
    "n_output_logits": None  # number of output classes (softmax)
}


# FEEDFORWARD EMBEDDING BASE - based on `base_model_dict`
...
# CONV EMBEDDING BASE - based on `base_model_dict`
cnn_embedding_model_dict = base_model_dict.copy()
cnn_embedding_model_dict.update({
    # ----------------------
    # Convolutional options:
    # ----------------------
    "n_filters": [128, 256],  # Two layers with 128->256 output filters
    "kernel_sizes": [(39, 9), (1, 8)],  # Two layers (39x9)->(1x8) filters (first layer should be (40, 9) for fbank)
    "conv_paddings": "valid",
    "pool_sizes": [(1, 3), (1, 17)], # Two layers (1x3)->(1x17) filters (collapses output to 1x256, invariant of segment length)
    "pool_type": "max",
...
# SIAMESE TRIPLET BASE - based on `cnn_embedding_model_dict`
...
def build_speech_model(
        ...
        y_train_labels=None,
        training=True):
    """Build the specified speech model, as well as loss func and metric ops.
    ...
    loss_func = None
    metrics = {}
    if model_version == 'dtw':
        # If model is dynamic time warping, only return embedding := input
        x_embed_input = tf.placeholder(TF_FLOAT, model_param_dict['x_input_shape'])
        embedding = x_embed_input
