...
"""Combine individual MFCC and FBank Flickr-Audio archives into single archive.
...
def check_arguments():
    ...
    parser.add_argument('--mfcc-train',
                        ...
                        required=True)
    parser.add_argument('--mfcc-dev',
                        type=str,
                        help="MFCC dev set NumPy archive",
                        required=True)
    ...
    parser.add_argument('--fbank-train',
                        ...
                        required=True)
    parser.add_argument('--fbank-dev',
                        type=str,
                        help="Filterbank dev set NumPy archive",
                        required=True)
...
def main():
    ...
    feats_files = [(ARGS.mfcc_train, ARGS.mfcc_dev, ARGS.mfcc_test), 
                   (ARGS.fbank_train, ARGS.fbank_dev, ARGS.fbank_test)]
    ...
    for feats_type in feats_files:
        ...
        for feats_file in feats_type:
            ...
            z_speakers = np.asarray(
                [key.split('_')[1] for key in k_keys], dtype=str)
